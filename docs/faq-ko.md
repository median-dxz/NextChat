...
OpenAI 공식 [Usage](https://platform.openai.com/usage) 및 [Billing](https://platform.openai.com/settings/organization/billing/overview) 페이지를 사용하세요. 타사 잔액 조회 사이트에 API 키를 입력하지 마세요.

## 새로 등록한 OpenAI 계정에 API 잔액이 없는 이유는 무엇인가요?

API 청구 상태와 프로모션 크레딧은 계정 및 OpenAI의 현재 정책에 따라 다릅니다. [Billing](https://platform.openai.com/settings/organization/billing/overview) 페이지에서 계정 상태를 확인하고, 새 계정에 무료 API 크레딧이 포함된다고 가정하지 마세요.
...
GPT-4 API는 더 이상 대기자 명단 신청 절차를 사용하지 않습니다. API 모델 사용 가능 여부는 계정과 프로젝트에 따라 다릅니다. API 키로 실제 사용할 수 있는 모델을 확인하고 최신 [모델 문서](https://developers.openai.com/api/docs/models)를 참고하세요. ChatGPT 구독과 API 청구는 별도로 관리됩니다.
...
- API 가격은 모델에 따라 다르며 변경될 수 있으므로 최신 [공식 가격 페이지](https://developers.openai.com/api/docs/pricing)를 확인하세요.
- 청구 금액이 예상보다 빠르게 증가하면 아래 단계에 따라 즉시 확인하세요:
...
청구 방식은 모델과 입력 유형에 따라 다르며 가격은 변경될 수 있습니다. 현재 [공식 API 가격 페이지](https://developers.openai.com/api/docs/pricing)를 확인하고 [Usage](https://platform.openai.com/usage) 페이지에서 실제 사용량을 확인하세요.

## OpenAI API 모델은 어떻게 선택하나요?

사용 가능한 모델과 권장 모델은 시간이 지나면서 변경됩니다. 최신 [모델 문서](https://developers.openai.com/api/docs/models)와 [지원 중단 안내](https://developers.openai.com/api/docs/deprecations)를 확인하고 API 키로 실제 사용할 수 있는 모델을 선택하세요.

...
公式の [Usage](https://platform.openai.com/usage) ページと [Billing](https://platform.openai.com/settings/organization/billing/overview) ページを使用してください。第三者の残高確認サイトに API キーを入力しないでください。

## OpenAI の新規アカウントに API 残高がないのはなぜですか？

API の請求状態やプロモーションクレジットは、アカウントと OpenAI の現在のポリシーによって異なります。[Billing](https://platform.openai.com/settings/organization/billing/overview) ページでアカウントの状態を確認し、新規アカウントに無料の API クレジットが付与されることを前提にしないでください。
...
GPT-4 API のウェイティングリスト手続きは現在ありません。API で利用できるモデルはアカウントとプロジェクトによって異なります。API キーで利用可能なモデルを確認し、最新の[モデルドキュメント](https://developers.openai.com/api/docs/models)を参照してください。ChatGPT のサブスクリプションと API の請求は別々に管理されます。
...
- API の価格はモデルによって異なり、変更されることがあります。最新の[公式価格ページ](https://developers.openai.com/api/docs/pricing)を確認してください。
- 請求額が予想より速く増えている場合は、以下の手順で直ちに確認してください:

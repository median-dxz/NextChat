import { FETCH_COMMIT_URL, FETCH_TAG_URL, ServiceProvider, StoreKey } from "../constant";
...
export const useUpdateStore = createPersistStore(
  ...
  (set, get) => ({
    ...
    async getLatestVersion(force = false) {
      ...
      try {
        ...
        if (window.__TAURI__ && isApp) {
          ...
          if (granted) {
            const isLatest = version === remoteId;
            sendNotification({
              title: "NextChat",
              body: isLatest
    ...
    async updateUsage(force = false) {
      ...
      try {
        const api = new ClientApi(ServiceProvider.OpenAI);
